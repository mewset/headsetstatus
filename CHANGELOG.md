...
### Fixed
- Tray icon now always has a default icon before being shown, preventing `QSystemTrayIcon::setVisible: No Icon set`.
- Empty-icon path now falls back to the headset icon so "no device" state still renders in tray.
